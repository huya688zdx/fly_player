...
- 布局口径（参照通用桌面播放器）：顶栏右侧只留一个**设置齿轮 icon**（点击打开设置抽屉）；**弹幕开关 / 倍速 / 选集 / 清晰度以文字与徽标按钮放底栏**（对应"弹 · 1.0x · 选集 · 4K"），后接字幕/音轨/书签/截图/小窗/全屏 icon。
- **选集为独立悬浮面板**（不放设置里）：顶栏下方右侧悬浮卡片，标题"剧名 · 第 N 季" + **列表/网格双视图切换**；列表视图为缩略图横向卡（集号. 标题 / 时长 / 进度条 / 已看勾，当前集高亮并自动滚动到位），网格视图为紧凑集数格子（进度条微缩 + 已看角标）；多季时顶部季芯片切换。切集后面板保持打开并刷新高亮。
- **设置抽屉**（右上角齿轮打开）只承载两组：`弹幕`（滚动/顶部/底部/彩色/去重/避让字幕 + 不透明度/密度/字号/速度/显示区域滑杆 + AI 防遮挡"规划中 onnxruntime"，对应 `DanmakuSettingsStore` 全部键；底栏弹幕开关**右键**直达）/ `播放设置`（解码模式/硬解 API、显示比例、deband/锐化/插帧、HDR 映射、缓存档位、音量增益、动态范围、性能悬浮窗，对应 `MpvSettingsStore` 的 mpv 属性）。
- **浮层菜单**（锚定按钮弹出）：清晰度（切换后跳回当前进度）、音轨、字幕（内嵌轨 + 外挂字幕 + ±0.5s 延时）、倍速、章节列表；**画面右键**聚合高频操作（播放暂停/章节/倍速/清晰度/书签/弹幕开关/弹幕设置/截图/小窗）——安卓长按动作表的键鼠形态。
- **进度条章节刻度**（OP/ED 金色标记 + 当前章节名实时显示）与**书签菱标**（`player_bookmarks_v1`）；OP 段内浮现**跳过片头**胶囊（对应 `intro_outro` 跳过），片尾 30s 浮现**自动连播倒计时卡**（对应 `auto_play_enabled`）。
- **中央 OSD** 反馈所有键盘操作：±10s/±30s seek、音量、亮度、倍速恢复；中央加载态（缓冲 + 预载秒数）对应安卓中央 loading；截图带白闪效果（对应 `FullscreenScreenshotActivity`）。
- **手势 → 键鼠映射**：左右滑动快进退 → ←/→（10s）与 Shift+←/→（30s）；长按临时双倍速 → **按住 → 键临时 3×**（松开恢复，带徽章提示）；上下滑动调音量/亮度 → **滚轮音量 / Alt+滚轮亮度**。
- **小窗模式（I 键）**：整个播放器缩为右下角 348×200 悬浮卡（弹幕继续），可返回媒体库浏览——桌面端对 PIP 的形态替代；性能悬浮窗（FPS/解码器/缓冲）对应 `player_performance_overlay`/`fps_overlay`。
- 快捷键全表：Space、←→/Shift+←→、按住→ 3×、↑↓/滚轮音量、Alt+滚轮亮度、N/P 切集、F 全屏、I 小窗、M 静音、S 截图、B 书签、D 弹幕、Esc 级联（菜单→选集面板→设置抽屉→小窗→退出）。
- 落地建议：Flutter 桌面端用 `media_kit`（libmpv）直接内嵌 Widget，播放逻辑仍复用 `ItemPlaybackLauncher` 解析出的 `MpvMediaSource`，不再经 MethodChannel 拉 Activity；弹幕为 Flutter 覆盖层（Picture 缓存口径，见 POC 计划），控制层与视频层用 `RepaintBoundary` 隔离。
