    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        ChangeNotifierProvider.value(
          value: account,
          child: const MaterialApp(home: FlyCatalogScreen()),
        ),
      );
...
class _ScopeService extends _CatalogService {
  ...
  Future<Map<String, dynamic>> request(String path, {
    Map<String, dynamic>? query, Object? body, bool patch = false,
  }) {
...
class _ScopeAccount extends FlyAccountController {
  _ScopeAccount(FlyDataService service) : super(
    nas: NasProvider(), backendSession: BackendSessionProvider(autoLoad: false),
    service: service, autoLoad: false,
  ) {
    activeBindingId = 'binding';
    bindings = [{'id': 'binding', 'label': '家里的飞牛'}];
  }
  void changeAccount() {
    service.session = FlyDataSession(serverUrl: 'https://test.example',
      userId: 'bob', username: 'bob', deviceId: 'd2', deviceName: 'd2',
      token: 'bob-token', installationId: 'i2', serviceInstanceId: 'instance');
    notifyListeners();
  }
  @override
  void dispose() {
...
void _scopeTests() {
  ...
  Widget host(Widget child) => ChangeNotifierProvider<FlyAccountController>.value(
    value: account, child: MaterialApp(home: child));
  Map<String, dynamic> page(String name) => {'items': [
    {'id': name, 'binding_id': 'binding', 'title': name, 'kind': 'series', 'remote_item_id': 'remote-$name'},
  ], 'total': 1, 'next_cursor': null};

  testWidgets('late catalog response cannot replace another account posters', (tester) async {
  ...
  testWidgets('poster never reuses the previous account image while next request waits', (tester) async {
    await tester.pumpWidget(host(const Scaffold(body: SizedBox(width: 160, height: 240,
      child: FlyCatalogPoster(mediaId: 'same-item')))));
    final oldPicture = Uint8List.fromList(base64Decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aXioAAAAASUVORK5CYII='));
  ...
  testWidgets('stale program action does not navigate after account changes', (tester) async {
    ...
    account.changeAccount();
    await tester.pump();
    await expectLater(openFlyCatalogItem(pageContext, {
      'binding_id': 'binding', 'remote_item_id': 'old-remote', 'kind': 'series',
    }, expectedAccountKey: originalAccount), throwsStateError);
