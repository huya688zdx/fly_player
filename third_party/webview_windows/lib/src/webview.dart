...
bool _isFlyIdentityUri(Uri? uri) {
  return uri != null &&
      uri.scheme == 'https' &&
      !uri.hasPort &&
...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// 返回固定 HTTPS FN 飞翔身份地址所需的 entry-token、mode 和 ost。
  /// 调用方不得记录这些值；保存时必须使用平台安全凭据存储。
  Future<List<Map<String, dynamic>>> getCookies(String uri) async {
    final target = Uri.tryParse(uri);
    if (!_isFlyIdentityUri(target)) {
      throw ArgumentError.value(uri, 'uri', '不是允许的飞翔身份地址');
    }
    if (_isDisposed) {
      return const <Map<String, dynamic>>[];
    }
    assert(value.isInitialized);
    final cookies = await _methodChannel.invokeListMethod<dynamic>(
      'getCookies',
      target.toString(),
    );
    return (cookies ?? const <dynamic>[])
        .map((dynamic cookie) {
          if (cookie is! Map) {
            throw const FormatException('原生 Cookie 结果格式无效');
          }
          return cookie.map<String, dynamic>(
            (dynamic key, dynamic value) => MapEntry(key as String, value),
          );
        })
        .where((cookie) =>
            const {'entry-token', 'mode', 'ost'}.contains(cookie['name']))
