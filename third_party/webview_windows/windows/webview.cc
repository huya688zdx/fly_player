...
void Webview::GetCookies(const std::string& uri, GetCookiesCallback callback) {
  static const std::regex allowed_uri(
      R"(^https://[A-Za-z0-9.-]+\.fnos\.net/app/fly-data-service/api/v1/system/identity$)");
  if (!IsValid() || !std::regex_match(uri, allowed_uri)) {
    callback(false, {});
    return;
  }
  ...
  const auto target_uri = util::Utf16FromUtf8(uri);
  const auto result = cookie_manager->GetCookies(
      target_uri.c_str(),
      Callback<ICoreWebView2GetCookiesCompletedHandler>(
          [callback](HRESULT error_code, ICoreWebView2CookieList* cookie_list)
              -> HRESULT {
            ...
            for (UINT32 index = 0; index < count; ++index) {
              ...
              const auto cookie_name = util::Utf8FromUtf16(name.get());
              if (cookie_name != "entry-token" && cookie_name != "mode" &&
                  cookie_name != "ost") {
                continue;
              }
              ...
              if (FAILED(cookie->get_Value(&value)) ||
                  FAILED(cookie->get_Domain(&domain)) ||
                  FAILED(cookie->get_Path(&path)) ||
                  FAILED(cookie->get_IsHttpOnly(&is_http_only)) ||
                  FAILED(cookie->get_IsSecure(&is_secure))) {
                continue;
              }
              ...
              cookies.push_back({
                  ...
                  static_cast<bool>(is_http_only),
                  static_cast<bool>(is_secure),
              });
            }
