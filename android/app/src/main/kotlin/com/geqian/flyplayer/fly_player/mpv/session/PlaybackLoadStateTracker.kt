...
class PlaybackLoadStateTracker {
    ...
    fun isSameLoadInFlight(sourceUrl: String, playbackUrl: String): Boolean {
        return sourceUrl.isNotBlank() &&
            playbackUrl.isNotBlank() &&
            loadingSourceUrl == sourceUrl &&
            activePlaybackUrl == playbackUrl
    }
    ...
    fun hasPendingOrInFlightLoad(sourceUrl: String): Boolean {
        if (sourceUrl.isBlank()) return false
        return (pendingLoadRequested && loadedSourceUrl != sourceUrl) ||
            loadingSourceUrl == sourceUrl
    }
