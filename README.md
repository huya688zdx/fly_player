...
基于 Flutter 与 mpv 开发的媒体播放器，连接 **飞牛影视、Emby 和 Jellyfin**。Android 使用 `mpv-android`，Windows、Linux、iOS、macOS 使用 `media_kit`。平台构建与验证边界见 [Apple 适配说明](docs/apple-platform-support.md) 和 [Linux 播放说明](docs/linux-playback.md)。
...
播放入口统一经过 `PlaybackHost`：Android 交给原生宿主与 `NativePlayerActivity`，Windows、Linux 和 Apple 平台交给 `DesktopPlaybackHost`。媒体后端负责解析播放源，平台宿主负责实际播放。
