...
import '../../playback/player_controls_auto_hide.dart';
import '../../playback/player_window_close_protection.dart';
...
class _DesktopPlaybackScreenState extends State<DesktopPlaybackScreen>
  ...
  late final PlayerControlsAutoHide _controlsAutoHide = PlayerControlsAutoHide(
    delay: _controlsHideDelay,
    canHide: () => mounted && _isPlaying && _controlsVisible,
    onHide: () => _updateView(() => _controlsVisible = false),
  );
  ...
  void _resetFlyBif() {
    ...
    final access = FlyBifAccess.capture(
      ...
      isCurrent: () =>
          ...
          generation == _flyBifGeneration &&
          identity.matches(_source),
    );
    _flyBifAccess = access;
    if (access == null || source.externalLocalSource) return;
    _flyActivity = FlyPlaybackActivity.forAccess(access)
      ..start(paused: source.startPaused);
    if (!source.supportsVerifiedFileOped) return;
    unawaited(
      FlyBifService.instance.resolve(access).then((path) {
        if (!access.isCurrent() || path == null) return;
        setState(() {
          _nasBifLocalPath = path;
        });
        _viewRevision.value++;
      }),
    );
  ...
  Object get _flyAccountIdentity =>
      (_flyClient.epoch, _flyAccountSignedIn, FlyDataService.instance.session);
  ...
  void initState() {
    ...
    _localStatsTimer = Timer.periodic(const Duration(seconds: 1), (_) {
  ...
  void _startAutomaticServiceDanmaku() {
    ...
    final source = _source;
    final prepareSeasonOnly = _danmakuComments.isNotEmpty;
    _autoDanmakuGeneration = _sourceChangeGeneration;
    final generation = _danmakuLoadGeneration;
    bool current() =>
        access.isCurrent() &&
        generation == _danmakuLoadGeneration &&
        _danmakuSettings.enabled &&
        (prepareSeasonOnly || _danmakuComments.isEmpty);
    if (!prepareSeasonOnly) {
    ...
    unawaited(() async {
      final path = await NativeDanmakuPrefetch.resolveOnPlaybackToFile(
        ...
        onStatus: (message) {
          if (!prepareSeasonOnly &&
              current() &&
              _serviceDanmakuStatus != message) {
            _updateView(() => _serviceDanmakuStatus = message);
          }
        },
      );
      if (prepareSeasonOnly || path == null || !current()) return;
  ...
  _SkipPromptKind? _computeSkipPromptKind(Duration position) {
    ...
    if (published != null) {
      final segment = published.at(
        position.inMilliseconds,
        enabled: _flyOpedAllowed,
      );
      if (segment == null || _flyAction != null) return null;
      if (segment.kind == 'op' && !_introSkipDismissed)
        return _SkipPromptKind.intro;
      if (segment.kind == 'ed' && !_outroSkipDismissed)
  ...
  void _scheduleControlsHide() => _controlsAutoHide.schedule();
  ...
  FlyOpedSet? get _currentFlyOped {
    if (!_flyOpedAllowed ||
        !_source.supportsVerifiedFileOped ||
        _flyScopeEpoch != _flyClient.epoch ||
        _flyClient.sourceRef(
              statsScope: _source.statsScope,
              itemGuid: _source.itemGuid,
              mediaGuid: _source.mediaGuid,
            ) ==
            null) {
  ...
  void _resetFlyOped({bool preserveTail = false}) {
    ...
    _flyContextId =
        'desktop-${_source.loadNonce}-${FlyPlaybackServiceClient.newId()}';
    _flyScopeEpoch = _flyClient.epoch;
  ...
  Future<void> _resolveFlyOped() async {
    ...
    final generation = _danmakuSeekRevision,
        sourceGeneration = _sourceChangeGeneration;
    final result = await _flyClient.resolve(
      statsScope: source.statsScope,
      itemGuid: source.itemGuid,
      mediaGuid: source.mediaGuid,
      contextId: contextId,
    ...
    if (!mounted ||
        contextId != _flyContextId ||
        sourceGeneration != _sourceChangeGeneration ||
        !_flyOpedAllowed ||
        generation != _danmakuSeekRevision ||
        !identical(source, _source) ||
        _flyScopeEpoch != _flyClient.epoch) {
      return;
    }
  ...
  Future<void> _skipPublished(
    FlyOpedSet set,
    FlyOpedSegment segment, {
    bool automatic = false,
  }) async {
    if (!_flyOpedAllowed ||
        _flyAction != null ||
        _flyAuthorizing ||
        !identical(set, _currentFlyOped) ||
        !segment.contains(_player.state.position.inMilliseconds))
      return;
    final generation = _danmakuSeekRevision,
        contextId = _flyContextId,
        source = _source;
    _flyAuthorizing = true;
    final authorized = await _flyClient.resolve(
      statsScope: source.statsScope,
      itemGuid: source.itemGuid,
      mediaGuid: source.mediaGuid,
      contextId: contextId,
    ...
    _flyAuthorizing = false;
    if (generation != _danmakuSeekRevision ||
        !identical(source, _source) ||
        !identical(set, _currentFlyOped))
      return;
    ...
    if (!_flyOpedAllowed ||
        (automatic && !_flyAutomaticAllowed) ||
        !segment.contains(_player.state.position.inMilliseconds))
      return;
    final action = FlyOpedAction(
      ...
      actionId: FlyPlaybackServiceClient.newId(),
    );
    if (segment.kind == 'ed') {
    ...
    unawaited(
      _flyClient.record(action.event('intent'), statsScope: _source.statsScope),
    );
    _dismissSkipPrompt();
    _flyActionTimeout = Timer(
      const Duration(seconds: 12),
      () => _finishFlyAction('failed'),
    );
    try {
      final command = _seekTo(
        Duration(milliseconds: segment.endMs),
      ...
      if (identical(action, _flyAction)) {
        ...
        _flyObservationTimer = Timer.periodic(
          const Duration(milliseconds: 250),
          (_) => _sampleFlyOpedAction(),
        );
      }
    } catch (_) {
      if (identical(action, _flyAction)) _finishFlyAction('failed');
    }
  }
  ...
  void _sampleFlyOpedAction() {
    ...
    if (action != null && !_flySamplePending && _flySeekCommandAccepted) {
      ...
      Future<void> observe() async {
        ...
        if (!mounted ||
            epoch != _danmakuSeekRevision ||
            !identical(action, _flyAction) ||
            !identical(set, _currentFlyOped) ||
            seconds == null ||
            !seconds.isFinite)
          return;
        final phase = action.sample(
          positionMs: (seconds * 1000).round(),
          engineAccepted: seeking == 'no' || seeking == 'false',
          generation: epoch,
        );
      ...
      unawaited(
        observe()
            ...
            .catchError((Object _) {})
            .whenComplete(() => _flySamplePending = false),
      );
    }
  }
  ...
  Widget _buildDanmakuSourcesPage(VoidCallback onApplied) =>
      DesktopDanmakuSourcePanel(
        ...
        onSearchFly: (keyword) =>
            _searchDanmakuSources(keyword, forceFly: true),
  ...
  void _toggleLock() {
    ...
    unawaited(setPlayerWindowPreventClose(_isLocked));
    _wakeControls();
    _showPlayerMessage(
  ...
  Widget _buildTouchControls(VideoState videoState) {
    return StreamBuilder<Duration>(
      ...
      builder: (context, duration) => StreamBuilder<Duration>(
        ...
        builder: (context, position) => TouchPlayerControls(
          title: _source.title,
          playing: _isPlaying,
          ...
          onFullscreen: () => unawaited(videoState.toggleFullscreen()),
          onSettings: () => unawaited(_showPlaybackSettingsPanel()),
          onSpeed: () => unawaited(_showTouchPlaybackSpeed()),
          onDanmaku: _toggleDanmaku,
          ...
          onNext: _nextEpisode == null
              ? null
              : () => unawaited(_showNextEpisode()),
          onQuality: _source.qualities.isEmpty || widget.reloadSource == null
              ? null
              : () => unawaited(_showQualities()),
          qualityLabel: _resolutionLabel,
        ),
      ),
    );
  }

  Future<void> _showTouchPlaybackSpeed() async {
    _wakeControls(scheduleHide: false);
    final rate = await showModalBottomSheet<double>(
      ...
      builder: (context) => ListView(
        shrinkWrap: true,
        children: [
          for (final rate in <double>[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.5, 3])
            ListTile(
              title: Text('$rate×'),
              selected: _playbackRate == rate,
              trailing: _playbackRate == rate ? const Icon(Icons.check) : null,
    ...
    if (!mounted) return;
    if (rate != null) await _setPlaybackRate(rate);
    _wakeControls();
  }
            ...
            child: Listener(
              onPointerDown: (event) {
                if (PlaybackPlatform.usesTouchControls &&
                  ...
                  _controlsAutoHide.pointerDown(event.pointer);
                }
              },
              onPointerUp: (event) =>
                  _controlsAutoHide.pointerEnded(event.pointer),
                  ...
                  children: <Widget>[
                    ...
                    IgnorePointer(
                      ...
                      child: AnimatedOpacity(
                        ...
                        child: PlaybackPlatform.usesTouchControls
                            ? _buildTouchControls(videoState)
                            : ValueListenableBuilder<
                                PlayerHoverOverlaySnapshot
                              >(
                                ...
                                builder: (context, hover, _) => DesktopPlayerControls(
                                  ...
                                  rate: _playbackRate,
                                  nowPlayingLabel:
                                      _l10n.nativeNotificationNowPlaying,
                                  ...
                                  muteTooltip: _volume > 0
                                      ? _l10n.desktopPlaybackMuteTooltip
                                      : _l10n
                                            .desktopPlaybackRestoreVolumeTooltip,
                                  speedTooltip: _l10n.playerDiagnosticsSpeed,
                                  fullscreenTooltip: videoState.isFullscreen()
                                      ? _l10n
                                            .desktopPlaybackExitFullscreenTooltip
                                      : _l10n.desktopPlaybackFullscreenTooltip,
                                  settingsTooltip:
                                      _l10n.desktopPlaybackMoreOptions,
                                  prevTooltip:
                                      _l10n.desktopPlaybackPrevEpisodeTooltip,
                                  bookmarkTooltip:
                                      _l10n.playerBookmarkAddCurrent,
                                  ...
                                  onEpisodesAt: _canBrowseEpisodes
                                      ...
                                      : null,
                                  onAudio: () =>
                                      unawaited(_showTracks(audio: true)),
                                  onSubtitle: () =>
      ...
      child: IgnorePointer(
        ...
        child: ValueListenableBuilder<_SkipPromptKind?>(
          ...
          builder: (context, kind, _) {
            ...
            final basis = _currentFlyOped != null
                ? '服务'
                : fromChapter
                ? '章节识别'
                : '固定时长';
