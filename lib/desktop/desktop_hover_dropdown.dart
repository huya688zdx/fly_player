                                        ...
                                        for (
                                          ...
                                          for (final item
                                            ...
                                            DesktopDropdownOptionRow(
                                              item: item,
                                              enabled: !spec.groups[i].disabledIds.contains(item.id),
                                              leading: spec.groups[i].leadingById[item.id],
                                              selected:
                                                  item.id ==
                                                  spec.groups[i].selectedId,
                                              onTap: () {
                                                final currentGroup = widget.spec?.groups.elementAtOrNull(i);
                                                if (!mounted || !_visible || currentGroup == null ||
                                                    currentGroup.disabledIds.contains(item.id) ||
                                                    !currentGroup.items.any((current) => current.id == item.id)) {
                                                  return;
                                                }
                                                _close();
