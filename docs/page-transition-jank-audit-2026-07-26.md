...
~~5. 实机验收~~：已完成（见"实机 A/B 验收"节）；仅 Emby 详情路径与
saveLayer 直接计数未覆盖。
...
## 实机 A/B 验收（2026-07-26）

**方法**：小米平板 2410CRP4CC（120Hz，帧预算 8.3ms）竖屏、profile 构建、
FN Connect（飞牛）后端；修复前基线 = `98c2e1c`（worktree 检出），修复后 =
`57e1c1c`+`3683a86`。同一路径（收藏页 → 白箱 TV 详情，进/出各 3 轮，adb 驱动、
点击后 900ms 窗口），VM service `Flutter.Frame` 事件逐帧采集 build/raster 耗时。

**raster 线程（掉帧主战场）**：

| 窗口 | 修复前 p90/avg (ms) · >16.7ms 帧 | 修复后 p90/avg (ms) · >16.7ms 帧 |
|---|---|---|
| 进详情①（冷） | 28.6 / 12.1 · 7 帧（max 69.0） | 19.2 / 7.4 · 6 帧（max 31.5） |
| pop① | 23.2 / 11.5 · 6 帧 | 19.4 / 7.0 · 5 帧 |
| 进详情②（热） | 30.1 / 13.6 · **9 帧** | 9.8 / 4.6 · **0 帧** |
| pop② | 17.5 / 10.3 · 4 帧 | 12.0 / 4.7 · **0 帧** |
| 进详情③ | 29.0 / 10.2 · 9 帧 | 13.7 / 6.7 · 3 帧 |
| pop③ | 15.7 / 9.6 · 3 帧 | 17.6 / 5.6 · 4 帧 |

**UI 线程（build）**：修复前各窗口 avg 2.4~4.1ms / p90 4.3~8.5ms，冷进有
3-4 帧超 8.3ms；修复后 avg 0.6~2.4ms / p90 ≤4.1ms，热路径 0 帧超帧预算。

**结论**：热路径（缓存命中的再次进出，日常最高频场景）修复后**完全干净**——
两线程 0 帧掉到 60fps 以下，raster p90 30.1→9.8ms（-67%）；冷进 raster 峰值
69→31.5ms 减半（剩余为首次图片纹理上传，属必要成本）。两版冷启首个详情页均有
一帧 ~55-65ms 的 UI 尖峰（AOT 首执/着色器，非本轮回归）。

**未覆盖**：Emby 详情路径（设备登录态为 FN Connect，切账号会清除当前连接，
留待 Emby 登录态时补测）；saveLayer 逐帧计数（Impeller 下无现成采集通道，
以帧耗时代替验收）。
...
- 实机 A/B 验收已完成（2026-07-26，见上节）：热路径 0 掉帧，冷进峰值减半。
