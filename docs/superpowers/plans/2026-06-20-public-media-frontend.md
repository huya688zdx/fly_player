...
- 每完成一个小任务并通过对应测试后，必须立刻单独提交。
- 不允许把多个小任务攒成一个大 commit。
- 提交前运行 `git status --short` 和 `git diff --cached --name-only`，确认只暂存当前小任务相关文件。
- 如果仓库里有其它未提交改动，保持原样，不回滚、不夹带。
- 测试失败时不要提交“完成”状态；先修复，或在状态看板写明阻塞、失败命令和错误摘要。
- 提交后更新 `docs/superpowers/public-media-frontend-status.md`，写入 commit hash、测试命令和下一步负责人；状态文档更新也可以作为同一个小任务的一部分提交。
