...
真机诊断确认 Flutter 已传递完整 `imageHeaders`，未下载集也存在有效 `poster`；失败发生在 Android 原生图片下载器的 TLS 握手阶段。NAS 图片地址使用私有或自签名证书，Flutter 网络层已有私网证书兼容策略，但原生 `OkHttpClient` 仍使用系统默认信任链，因此请求抛出 `SSLHandshakeException`，Glide 最终只显示黑色占位背景。本地封面不经过网络，所以始终正常。

修复证书后又观察到面板轻微闪烁：每次打开选集都会发起一次静默状态刷新，即使回包内容没有变化也会无条件重建整页列表，导致已显示的 Glide 图片视图被重新绑定一次。
...
- 携带 NAS 鉴权头的 HTTPS 图片兼容私有、自签名证书及自定义域名；无鉴权头的普通公网图片仍使用系统证书校验。
- 静默刷新内容未变化时不重建选集面板，消除打开后的二次闪烁。
...
`NativeSafeImageHttp` 仅在请求同时满足“HTTPS URL”和“存在已过滤的 NAS 鉴权头”时使用兼容私有证书的客户端。带鉴权头的跳转仍由现有策略手动处理且只允许同源；HTTP 地址和无鉴权头图片不会进入该客户端。

选集静默刷新先合并最新观看状态，仅当加载态需要收起或列表内容实际变化时才调用 `renderTopPanel()`。内容稳定时保留原有 View 与 Glide 请求，不做第二次整页绑定。

## 安全与错误处理

- Flutter 只为飞牛 NAS 同源图片生成 NAS 鉴权头；本地文件和第三方图片不附加 NAS 凭据。
- Android 不记录请求头内容，也不把请求头拼进 URL。
- 私有证书兼容只用于携带 NAS 鉴权头的 HTTPS 图片；同源重定向、请求头过滤和图片大小上限保持不变。
...
- Kotlin TLS 策略测试：覆盖私网、CGNAT、IPv6 本地地址，以及携带鉴权头时的自定义 HTTPS 域名；HTTP 和无效 URL 保持拒绝。
- 面板刷新测试：内容稳定且非加载态时不重绘，加载态收起或内容变化时仍重绘。
