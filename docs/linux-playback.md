...
2026-09-15 合入当前主线时，已补齐 Linux 的统一起播防误触与取消清理，并保留后续直播、外部播放器和会话所有权逻辑。此次在 Windows 上通过 50 项相关现有测试和 12 个文件的静态检查；未重新执行 Linux 设备验证，下面的 Ubuntu 结果属于 2026-09-10 的历史记录。另有一项现有 NAS 超时请求次数测试失败，已在不含 Linux 改动的主线基线上复现，未纳入本次修复范围。
